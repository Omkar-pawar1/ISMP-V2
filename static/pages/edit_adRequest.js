const edit_adRequest={
    template: `
    <div>
      <h1>Editing Ad Request </h1>
  ...
  methods: {
    ...
    async makeAdRequest() {
      try {
        const res = await fetch(`${window.location.origin}/edit_ad_request/${this.currentCampaign_id}`, {
          method: 'PUT',
          headers: {
            AuthenticationToken: sessionStorage.getItem('token'),
